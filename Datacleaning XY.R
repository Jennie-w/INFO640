...
meals <- read.csv("Desktop/info640-labs/DataSets/mealplan.csv", skip=3, header=TRUE, check.names = FALSE)
...
meals1$location <- str_trim(meals$Location)
...
maxprice <- max(meal2$price, na.rm=TRUE)
print(maxprice)
